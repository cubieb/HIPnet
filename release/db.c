...
//HIPNET__ Add down v6 and v4 down routes for sub-delegated prefix
void
set_route(const struct ia_xx *ia) {

    static char v6_down_cmd[100] = "";
    static char v4_down_cmd[100] = "";
    int stat;

    // IA-NA and IA-PD access code copied from
    // the function that called me below, write_ia(const struct ia_xx *ia).
    ...
    switch (ia->ia_type) {
        case D6O_IA_NA:
            inet_ntop(AF_INET6, &iasubopt->addr, ia_na, sizeof(ia_na));
            break;

        case D6O_IA_PD:
            iasubopt = ia->iasubopt[0];
            inet_ntop(AF_INET6, &iasubopt->addr, addr_buf, sizeof(addr_buf));

            // delete existing v6 down route to
            // avoid multiple entries in the routing table
            if (strcmp(v6_down_cmd, "") != 0) {
                //log_info("HIPnet, route v6_down_cmd, delete: %s", v6_down_cmd);
                stat = system(v6_down_cmd);
                log_info("HIPnet, system(route v6_down_cmd, delete) stat: %d", stat);
            }

            // add v6 down route
            sprintf(v6_down_cmd, "route -A inet6 add %s/%d gw %s dev br-lan",
                                 addr_buf, (int)iasubopt->plen, ia_na);

            //log_info("HIPnet, route v6_down_cmd, add: %s", v6_down_cmd);
            stat = system(v6_down_cmd);
            log_info("HIPnet, system(route v6_down_cmd, add) stat: %d", stat);

            // store v6 down route delete command
            sprintf(v6_down_cmd, "route -A inet6 del %s/%d gw %s dev br-lan",
                                 addr_buf, (int)iasubopt->plen, ia_na);



            // delete existing v4 down route
            if (strcmp(v4_down_cmd, "") != 0) {
                //log_info("HIPnet, route v4_down_cmd, delete: %d", v4_down_cmd);
                stat = system(v4_down_cmd);
                log_info("HIPnet, system(route v4_down_cmd, delete) stat: %d", stat);
            }

            // generate down router LAN v4 ip
            // first octet is always '10'
            char octet2[3];
            char octet3[3];
            int  last_char = strlen(addr_buf);

            octet2[0] = addr_buf[last_char - 6];
            octet2[1] = addr_buf[last_char - 5];
            octet2[2] = '\0';
            int lan_octet2_int = strtol(octet2, NULL, 16);

            octet3[0] = addr_buf[last_char - 4];
            octet3[1] = addr_buf[last_char - 3];
            octet3[2] = '\0';
            int lan_octet3_int = strtol(octet3, NULL, 16);

            // netmask for LAN v4
            const char* netmask = get_netmask((int)iasubopt->plen);


            // generate down router WAN v4 ip
            // IR host id is always one character
            // first octet is always '10'
            char host_id[2] = "";

            octet2[0] = ia_na[last_char - 6];
            octet2[1] = ia_na[last_char - 5];
            octet2[2] = '\0';
            int wan_octet2_int = strtol(octet2, NULL, 16);

            octet3[0] = ia_na[last_char - 4];
            octet3[1] = ia_na[last_char - 3];
            octet3[2] = '\0';
            int wan_octet3_int = strtol(octet3, NULL, 16);

            // host identifier is always one character 
            host_id[0] = ia_na[last_char];
            host_id[1] = '\n';
            int host_id_int = strtol(host_id, NULL, 16);

            // add v4 down route
            sprintf(v4_down_cmd, "route -A inet add -net 10.%d.%d.0 netmask %s gw 10.%d.%d.%d",
                                 lan_octet2_int, lan_octet3_int, netmask,
                                 wan_octet2_int, wan_octet3_int, host_id_int);

            //log_info("HIPnet, route v4 down route command, add: %s", v4_down_cmd);
            stat = system(v4_down_cmd);
            log_info("HIPnet, system(route v4_down_cmd, add) stat: %d", stat);

            // store v4 down route command
            sprintf(v4_down_cmd, "route -A inet del -net 10.%d.%d.0 netmask %s gw 10.%d.%d.%d",
                                 lan_octet2_int, lan_octet3_int, netmask,
                                 wan_octet2_int, wan_octet3_int, host_id_int);
            break;
    }
