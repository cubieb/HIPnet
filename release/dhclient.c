 ...
 *
 * Copyright © 2014 modifications made by CableTelevision Laboratories, Inc.
 *
 ...
 *
 * Modifications were made by CableLabs to support HIPnet, learn more about HIPnet at
...
TIME default_lease_time = 200;
TIME max_lease_time = 400;
//__HIPNET
...
//HIPNET__
char*
get_netmask(const int prefix)
{
    char* netmask = NULL;

    //IPv6 link bits are 40 bits away from IPv4 Class-A 2nd octet
    switch(prefix) {
        case 52 :          // 52 - 40 == 12
            netmask = "255.240.0.0";
            return netmask;
...
//HIPNET__
    // WARNING: This code supports one down HIPnet IR. If more than one router
    // is connected to this router's LAN, then connect only one router and reboot.

    // prevent accumulating duplicate ips and routes due to lease renewal
    // Note, down routes set in ../server/db.c.
    static char v6_ip_cmd[100] = "";
    static char v6_up_cmd[100] = "";
    static char v4_up_cmd[200] = "";

    // rx_* is address or prefix delegated from up router
    char rx_iana[100] = ""; // for "/48 Check" and adding WAN v4 default up route
    char rx_iapd[100] = ""; // for "/48 Check" and generating LAN v4 ip
    int  rx_iapd_size = 0;
    int  rx_iapd_chars = 0;
    char rx_link_id[5] = "";

    // tx_* is address or prefix delegated to down router
    char tx_iapd[100] = ""; // sub-delegated prefix
    int  tx_iapd_size = 0;
    char tx_link_id[5] = "";
    ...
    stat = system("/etc/init.d/dhclient stop");
    ...
    // leases 
    // TODO, Determine benefits of modifying default lease management.

    // dhcpd6.conf and dhcpd.conf are re-written, and servers restarted, below
    // TODO, determine way to write configuration to memory instead of server restart
    // TODO, make servers reconfiguration and restart contingent to change in prefix delegation
    stat = system("/etc/init.d/dhcpd6 stop");
    log_info("HIPnet, system(dhcpd6 stop) stat: %d", stat);

    stat = system("/etc/init.d/dhcpd stop");
    log_info("HIPnet, system(dhcpd stop) stat: %d", stat);
//__HIPNET
    ...
    for (ia = lease->bindings ; ia != NULL ; ia = ia->next) {
        ...
        if (stat <= 0) return ISC_R_IOERROR;
        ...
        for (addr = ia->addrs ; addr != NULL ; addr = addr->next) {
            ...
            if (ia->ia_type != D6O_IA_PD) {
                ...
                if (stat <= 0) return ISC_R_IOERROR;
//HIPNET__
                // Note given ia_na is piaddr(addr->address) when D6O_IA_NA
                strcpy(rx_iana, piaddr(addr->address));
//__HIPNET

            } else {  // we have a delegated prefix

                // lease file
                stat = fprintf(leaseFile, "    iaprefix %s/%d {\n", piaddr(addr->address), (int)addr->plen);
                if (stat <= 0) return ISC_R_IOERROR;

//HIPNET__
                // Note delegated ia_pd is piaddr(addr->address) when D6O_IA_PD
                strcpy(rx_iapd, piaddr(addr->address));
                rx_iapd_size = (int)addr->plen;


                /* Start prefix sub-delegation
                    Known
                    ...
                    I. Determine sub-delegated prefix at top of address pool
                ...
                */
                ...
                // A. known values
                // TODO, support more than one connected down router
                int lan_count = 1;
                if (rx_iapd_size < 52) {
                    log_error("HIPnet, this version of HIPnet only supports given prefixes /52 or smaller.");
                }
                rx_iapd_chars = strlen(piaddr(addr->address));


                // B. configuration mode: depth or width
                int depth = 0;
                int width = 0;
                if (lan_count >= 8) {
                    width = 1;
                ...
                // C. bit_boundary used to determine the number of subnets
                int bit_boundary = 0;
                if (rx_iapd_size <= 56) {
                    bit_boundary = 3;
                } else {
                ...
                // D. number of subnets is 2^bit_boundry
                // (e.g. bit_boundry == 3 then number_subnets == 2^3)
                int number_subnets = 1 << bit_boundary;
                //log_info("HIPnet, number_subnets: %d", number_subnets);


                // E. prefix sub-delegation size in bits for determining number of networks
                int subnet_bits = 64 - rx_iapd_size;
                //log_info("HIPnet, subnet_bits: %d", subnet_bits);


                // F. determine the number of networks;
                //    divide prefix by 4, 8,or 16 based on 2, 3, or 4 bit boundary (2^subnet_bits)
                int number_networks = 1 << subnet_bits;
                //log_info("HIPnet, number_networks: %d", number_networks);


                // G. number of /64 networks in a sub-delegated prefix,
                //    used to determine the sub-delegated prefix
                int tx_prefix = number_networks / number_subnets;
                //log_info("HIPnet, tx_prefix: %d", tx_prefix);


                // H. the size of the sub-delegated prefixes
                //    and the char array position of the significant char
                //    of the last quad A of the given delegated prefix
                //    This version of HIPnet only supports /52 and smaller prefixes.
                int quad_A_index = 0;

                if (rx_iapd_size > 64) {
                    log_error("HIPnet, delegated prefix % is not supported.", rx_iapd_size);
                } else if (rx_iapd_size >= 62) {
                    tx_iapd_size = 64;
                    quad_A_index = 1;
                } else if (rx_iapd_size >= 60) {
                    tx_iapd_size = rx_iapd_size + 2;
                    quad_A_index = 1;
                } else if (rx_iapd_size > 56) {
                    tx_iapd_size = rx_iapd_size + 2;
                    quad_A_index = 2;
                } else if (rx_iapd_size == 56) {
                    tx_iapd_size = rx_iapd_size + 3;
                    quad_A_index = 2;
                } else if (rx_iapd_size >= 52) {
                    tx_iapd_size = rx_iapd_size + 3;
                    quad_A_index = 3;
                } else {
                    log_error("HIPnet, delegated prefix % is not supported.", rx_iapd_size);
                ...
                // I. sub-delegated prefix at top of address pool
                // J. sub-delegated prefix range

                // create lower bound of prefix; also used in CER detection
                char prefix_floor[rx_iapd_chars];
                prefix_floor[rx_iapd_chars - 1] = '\0';
                strncpy(prefix_floor, piaddr(addr->address), rx_iapd_chars);
                //log_info("HIPnet, prefix_floor, before: %s", prefix_floor);

                // modify lower bound of prefix
                memcpy(rx_link_id, prefix_floor + (rx_iapd_chars - 6), 4);
                rx_link_id[4] = '\0';
                //log_info("HIPnet, rx_link_id: %s", rx_link_id);

                // convert char array to hex integer
                int rx_link_id_int = (int) strtol(rx_link_id, 0, 16);

                // prefixes are delegated starting at the top of the pool
                rx_link_id_int += tx_prefix * (number_subnets - 1);

                // generate sub-delegated Link-ID; also used in v4 provisioning
                snprintf(tx_link_id, 5, "%x", rx_link_id_int);

                // modify prefix_floor to top of the pool
                memcpy(prefix_floor + (rx_iapd_chars - 6), tx_link_id, 4);
                //log_info("HIPnet, prefix_floor: %s", prefix_floor);

                // tx_iapd used in provisioning v4 
                sprintf(tx_iapd, "%s::", prefix_floor);


                // create upper bound of prefix
                char prefix_ceiling[rx_iapd_chars - 2];
                prefix_ceiling[rx_iapd_chars - 1] = '\0';
                strncpy(prefix_ceiling, piaddr(addr->address), rx_iapd_chars - 2);

                // prefix_ceiling always ends in 'f's
                int tail;
                for (tail = 1; tail <= quad_A_index; tail++) {
                    prefix_ceiling[rx_iapd_chars - (tail + 2)] = 'f';
                }
                //log_info("HIPnet, prefix_ceiling: %s", prefix_ceiling);
                ...
                for (ip = interfaces; ip; ip = ip->next) {
                    ...
                    // delete default up route
                    if (strcmp(v6_up_cmd, "") != 0) {
                        //log_info("HIPnet, route v6_up_cmd, delete: %s", v6_up_cmd);
                        stat = system(v6_up_cmd);
                        log_info("HIPnet, system(route v6_up_cmd, delete) stat: %d", stat);
                    }


                    // caclulate default up route
                    char def_up[rx_iapd_chars - 1];
                    def_up[rx_iapd_chars - 2] = '\0';
                    strncpy(def_up, rx_iana, rx_iapd_chars - 2);  // 2 for "::"
                    def_up[rx_iapd_chars - (quad_A_index + 2)] = '0';

                    // add default up route
                    sprintf(v6_up_cmd, "route -A inet6 add ::/0 gw %s::1 dev eth1", def_up);
                    //log_info("HIPnet, route v6_up_cmd, add: %s", v6_up_cmd);
                    stat = system(v6_up_cmd);
                    log_info("HIPnet, system(route v6_up_cmd, add) stat: %d", stat);

                    // store default up route delete command
                    sprintf(v6_up_cmd, "route -A inet6 del ::/0 gw %s::1 dev eth1", def_up);


                    // delete LAN ip
                    if (strcmp(v6_ip_cmd, "") != 0) {
                        //log_info("HIPnet, ifconfig v6_ip_cmd %s", v6_ip_cmd);
                        stat = system(v6_ip_cmd);
                        log_info("HIPnet, system(ifconfig v6_ip_cmd, delete) stat: %d", stat);
                    }

                    // add LAN ip 
                    sprintf(v6_ip_cmd, "ifconfig br-lan add %s1/64", piaddr(addr->address));
                    //log_info("HIPnet, ifconfig v6_ip_cmd, add): %s", v6_ip_cmd);
                    stat = system(v6_ip_cmd);
                    log_info("HIPnet, system(ifconfig v6_ip_cmd, add) stat: %d", stat);

                    // store LAN ip delete command
                    sprintf(v6_ip_cmd, "ifconfig br-lan del %s1/64", piaddr(addr->address));


                    // v6 LAN specific down route added in ../server/db.c.


                    // write dhcpd6.conf file
                    FILE *dhcpd6_conf_file = NULL;
                    dhcpd6_conf_file = fopen("/etc/dhcpd6.conf", "w");;
                    if (dhcpd6_conf_file == NULL) {
                        log_error("can't create /etc/dhcpd6.conf file");
                        return ISC_R_IOERROR;
                    }

                    fprintf(dhcpd6_conf_file, "authoritative;\n");
                    //fprintf(dhcpd6_conf_file, "default-lease-time 3600;\n");
                    //fprintf(dhcpd6_conf_file, "max-lease-time 7200;\n");
                    fprintf(dhcpd6_conf_file, "default-lease-time 200;\n");
                    fprintf(dhcpd6_conf_file, "max-lease-time 400;\n");

                    fprintf(dhcpd6_conf_file, "subnet6 %s/%d {\n",
                                              piaddr(addr->address), rx_iapd_size);

                    fprintf(dhcpd6_conf_file, "\trange6 %s2", piaddr(addr->address));
                    fprintf(dhcpd6_conf_file, " %sf;\n", piaddr(addr->address));

                    fprintf(dhcpd6_conf_file, "\tprefix6 %s0", prefix_floor);
                    fprintf(dhcpd6_conf_file, " %s:ffff:ffff:ffff:ffff /%d;\n",
                                              prefix_ceiling, tx_iapd_size);

                    fprintf(dhcpd6_conf_file, "}\n");

                    fflush(dhcpd6_conf_file);
                    fclose(dhcpd6_conf_file);

                    // dhcpd6 restart
                    stat = system("/etc/init.d/dhcpd6 start");
                    log_info("HIPnet, system(dhcpd6 start) stat: %d", stat);

//__HIPNET, end recursive prefix delegation

                }  // end for(ips)
            }  // end else we have an IA_PD
            ...
            if (stat <= 0)
                return ISC_R_IOERROR;
        }  // end for each address in lease binding
...
//HIPNET__
    ...
    // Determine if this router is CER or IR using the HIPnet "/48" Check
    // "/48" Check - doe the IANA and IAPD come from the same addres pool?  If no then router is CER.
    // TODO, The code below is really the "/52" Check.  Determine if "/52" Check is legal?
    // TODO, Verify the generation and comparison IA-NA and IA-PD are correct. 

    int is_CER_router = strncmp(rx_iana, rx_iapd, rx_iapd_chars - 5);
    log_info("HIPnet, is_CER_router, 0 indicates no: %d", is_CER_router);


    // demonstrate CER vs. IR funtionality, CER firewall ON, IR fireall OFF
    char command[200] = "";

    if (is_CER_router == 0) {
        sprintf(command, "/etc/init.d/firewall stop");
        stat = system(command);
        log_info("HIPnet, system(firewall stop) stat: %d", stat);

        // Note, here would be a good place to turn off NAT.

    } else {
        sprintf(command, "/etc/init.d/firewall start");
        stat = system(command);
        log_info("HIPnet, system(firewall start) stat: %d", stat);
    }



    // Provision v4 WAN and LAN.
    // algorithm based on smallest possible prefix delegation of /64
    // resulting in smallest possible netmask of 255.255.255.0

    // WAN v4 IP and default up route 
    // If CER, then WAN v4 IP set from up router dhcpd, 
    // and v4 default up route set from dhcpd routers option.
    // If IR, then WAN v4 IP and default up route are added here.
    ...
    if (is_CER_router == 0) {
        ...
        // generate v4 WAN address
        // if IR, then host id is always one character
        int last_char = strlen(rx_iana);

        octet2[0] = rx_iana[last_char - 7];
        octet2[1] = rx_iana[last_char - 6];
        octet2[2] = '\0';
        int wan_octet2_int = strtol(octet2, NULL, 16);

        octet3[0] = rx_iana[last_char - 5];
        octet3[1] = rx_iana[last_char - 4];
        octet3[2] = '\0';
        int wan_octet3_int = strtol(octet3, NULL, 16);

        // IR host identifier is always one character
        char host_id[2] = "";
        host_id[0] = rx_iana[last_char - 1];
        host_id[1] = '\n';
        int host_id_int = strtol(host_id, NULL, 16);

        // reset WAN v4 address
        char v4_wan_cmd[200] = "";
        sprintf(v4_wan_cmd, "ifconfig eth1 add 10.%d.%d.%d netmask 255.255.255.0",
                            wan_octet2_int, wan_octet3_int, host_id_int);
        //log_info("HIPnet, ifconfig v4 WAN IP add cmd): %s", v4_wan_cmd);
        stat = system(v4_wan_cmd);
        log_info("HIPnet, system(ifconfig v4 WAN IP add cmd) stat: %d", stat);


        // delete WAN v4 default up route
        if (strcmp(v4_up_cmd, "") != 0) {
            //log_info("HIPnet, route v4_up_cmd, delete: %s", v4_up_cmd);
            stat = system(v4_up_cmd);
            log_info("HIPnet, system(route v4_up_cmd, delete) stat: %d", stat);
        }

        // add WAN v4 default up route
        sprintf(v4_up_cmd, "route -A inet add default gw 10.%d.%d.1", wan_octet2_int, wan_octet3_int);
        //log_info("HIPnet, route v4 WAN up add cmd: %s", v4_up_cmd);
        stat = system(v4_up_cmd);
        log_info("HIPnet, system(route v4 WAN up add cmd) stat: %d", stat);

        // store WAN v4 default up route delete command
        sprintf(v4_up_cmd, "route -A inet del default gw 10.%d.%d.1", wan_octet2_int, wan_octet3_int);
    }


    // generate LAN v4 IP
    // tx_link_id set in v6 provisioning section
    octet2[0] = rx_link_id[0];
    octet2[1] = rx_link_id[1];
    octet2[2] = '\0';
    int lan_octet2_int = strtol(octet2, NULL, 16);

    octet3[0] = rx_link_id[2];
    octet3[1] = rx_link_id[3];
    octet3[2] = '\0';
    int lan_octet3_int = strtol(octet3, NULL, 16);

    // determine netmask for LAN v4
    const char* netmask = get_netmask(tx_iapd_size);

    // add LAN v4 IP
    sprintf(command, "ifconfig br-lan add 10.%d.%d.1 netmask 255.255.255.0", lan_octet2_int, lan_octet3_int);
    //log_info("HIPnet, ifconfig v4 LAN IP add cmd): %s", command);
    stat = system(command);
    log_info("HIPnet, system(ifconfig v4 LAN IP add cmd) stat: %d", stat);


    // v4 LAN specific down route added in ../server/db.c.


    // write dhcpd.conf file
    FILE *dhcpd_conf_file = NULL;
    dhcpd_conf_file = fopen("/etc/dhcpd.conf", "w");
    if (dhcpd_conf_file == NULL) {
        log_error("can't create /etc/dhcpd.conf file");
        return ISC_R_IOERROR;
    }

    //fprintf(dhcpd_conf_file, "default-lease-time 3600;\n");
    //fprintf(dhcpd_conf_file, "max-lease-time 7200;\n");
    fprintf(dhcpd_conf_file, "default-lease-time 400;\n");
    fprintf(dhcpd_conf_file, "max-lease-time 800;\n");

    fprintf(dhcpd_conf_file, "option subnet-mask 255.255.255.0;\n");

    fprintf(dhcpd_conf_file, "option broadcast-address 10.%d.%d.255;\n",
                             lan_octet2_int, lan_octet3_int);

    fprintf(dhcpd_conf_file, "option routers 10.%d.%d.1;\n",
                             lan_octet2_int, lan_octet3_int);

    fprintf(dhcpd_conf_file, "option domain-name-servers 8.8.8.8, 8.8.4.4;\n");
    fprintf(dhcpd_conf_file, "option domain-search \"hipnet.cablelabs.org\";\n");

    fprintf(dhcpd_conf_file, "subnet 10.%d.%d.0 netmask 255.255.255.0 {\n",
                             lan_octet2_int, lan_octet3_int);

    fprintf(dhcpd_conf_file, "range 10.%d.%d.100 10.%d.%d.200;\n",
                             lan_octet2_int, lan_octet3_int, lan_octet2_int, lan_octet3_int);

    fprintf(dhcpd_conf_file, "}\n");

    fflush(dhcpd_conf_file);
    fclose(dhcpd_conf_file);

    // dhcpd restart
    stat = system("/etc/init.d/dhcpd start");
    log_info("HIPnet, system(dhcpd start) stat: %d", stat);
//__HIPNET
